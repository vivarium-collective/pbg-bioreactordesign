...
class BioreactorPlots(Visualization):
    """Time-series HTML plot of BiRD bioreactor scalar outputs.

    Consumes the four core bioreactor scalars (dissolved_o2,
    dissolved_co2, biomass, kla_o2) at each step, accumulates them
    across calls, and emits a Plotly HTML figure on every update.
    Downstream consumers (dashboards, notebook viewers) read the latest
    'html' from the wired store.
    """

    config_schema = {
        'title': {'_type': 'string', '_default': 'BiRD bioreactor'},
    }
    ...
    def update(self, state, interval=1.0):
        ...
        title = (self.config or {}).get('title', 'BiRD bioreactor')
        traces = []
